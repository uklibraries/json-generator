...
@mets.xpath('//mets:div', @namespaces).each do |node|
    if (node.xpath('mets:fptr').count == 0) and (node.xpath('mets:div').count == 0)
        has_error = true
        puts "not ok #{@id} : #{node.to_xml[0, 80]}"
        break
    elsif node['TYPE'] == 'section' and (node.xpath('mets:div').count == 0)
        has_error = true
        if node.xpath('mets:fptr').count > 0
            puts "not ok #{@id} : missing div level in #{node.to_xml[0, 40]}"
        else
            puts "not ok #{@id} : #{node.to_xml[0, 80]}"
        end
        break
